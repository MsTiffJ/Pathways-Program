...
async function getMeat() {
    //build the call to the API string by URL
    let apiString = "https://baconipsum.com/api/";
    //add the parameters to the string using the drop down lists
    let brandNewType = document.getElementById("newType").value;
    let brandNewParagraphs = document.getElementById("newParagraphs").value;
    let brandNewSentences = document.getElementById("newSentences").value;
    if (brandNewSentences == "6") {
        apiString = apiString + `?type=${brandNewType}&paras=${brandNewParagraphs}`;
    }
    else {
        apiString = apiString + `?type=${brandNewType}&sentences=${brandNewSentences}`;
    }
    //make the API call to the web service using the string and store what is returned
    ...
    let theNewFormat = document.getElementById("newFormat").value;
    var newJsonData = "";
    if (theNewFormat === "1")
...
function cipher1(someJSON) {
    ...
    let newChar; // will contain the character being examined
    let newCharCode; // will contain the ASCII code of the character being examined
    ...
    for (let para in someJSON) { // for each paragraph in the object
